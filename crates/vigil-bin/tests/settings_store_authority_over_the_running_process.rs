...
impl Deployment {
    ...
    /// Readiness is the control socket accepting a connection — a state, never
    /// a sleep and never a printed line.
    ...
    /// The inverse wait, so the next start is genuinely a second process.
